...
func TestClient_GetBlockByID(t *testing.T) {
	server, closer := mockJSONRPC(t, json.RawMessage(`{
  "jsonrpc": "2.0",
  "result": {
  ...
  "id": 123
	...
	res, err := client.GetBlock(context.Background(), "123abc")
	require.NoError(t, err)

	assert.Equal(t, map[string]interface{}{"id": float64(0), "jsonrpc": "2.0", "method": "block", "params": map[string]interface{}{"block_id": "123abc"}}, server.RequestBody(t))
	assert.NotNil(t, res)
	assert.Equal(t, res.Header.Height, 17821130)
}
